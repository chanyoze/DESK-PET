...
| 산탄총 | `%occultist3` (1,0) | `%occultist4` 1·2행 12프레임 | 평소 |
| 권총 | `%occultist3` (1,4) | `%occultist4` 5·6행 12프레임 | 평소 |

`%occultist3` 은 3프레임 걷기(+대각), `%occultist4` 는 같은 걷기를 12프레임으로 부드럽게 그린 판이다.
걷기는 12프레임판만 쓰고, 서 있는 칸만 `%occultist3` 에서 가져온다.
...
**껴안기**: `$Samarie_duppel` 2행은 사마리(왼쪽)가 마리나(오른쪽)를 껴안고 흔드는 3프레임이다.
둘을 같이 띄우면 사마리가 가끔 마리나에게 달려가 이 그림으로 바뀐다 (그동안 마리나는 숨긴다).
